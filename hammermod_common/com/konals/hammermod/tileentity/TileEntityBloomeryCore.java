...
public class TileEntityBloomeryCore extends TileEntity implements ISidedInventory {
    ...
    public void invalidateMultiblock() {
        ...
        metadata = metadata & BlockBloomeryCore.MASK_DIR;
    ...
    public boolean checkIfProperlyFormed() {
        ...
        int dir = (getBlockMetadata() & BlockBloomeryCore.MASK_DIR);

        int depthMultiplier = ((dir == BlockBloomeryCore.META_DIR_NORTH || dir == BlockBloomeryCore.META_DIR_WEST) ? 1 : -1);
        boolean forwardZ = ((dir == BlockBloomeryCore.META_DIR_NORTH) || (dir == BlockBloomeryCore.META_DIR_SOUTH));
        ...
        for (int horiz = -1; horiz <= 1; horiz++) // Horizontal (X or Z)
        {
            for (int vert = -1; vert <= 1; vert++) // Vertical (Y)
            {
                for (int depth = 0; depth <= 2; depth++) // Depth (Z or X)
                {
                    ...
                    worldObj.setBlock(x, y, z, HammerMod.bloomeryDummy.blockID);
                    worldObj.markBlockForUpdate(x, y, z);
                    TileEntityBloomeryDummy dummyTE = (TileEntityBloomeryDummy) worldObj.getBlockTileEntity(x, y, z);
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (isActive == 0 && furnaceBurnTime > 0) {
                ...
                isActive = 1;
                metadata = (isActive << 3) | (metadata & BlockBloomeryCore.META_ISACTIVE);
    ...
    public String getInvName() {
        ...
        return "Bloomery.container.bloomerycore";
    }
